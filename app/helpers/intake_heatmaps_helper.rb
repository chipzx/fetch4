module IntakeHeatmapsHelper
  ...
  def map_intakes2(map_detail, map_id, intakes, center_point, zoom)
    ...
    intakes.to_a.each do |a|
      ...
      # Need to call escape_javascript on found_location because street names and addresses may be quoted
      str += "#{id}.bindPopup('#{a.animal_id}<br/>#{a.animal_type} - #{a.intake_type}<br/>#{j(a.found_location)}<br/>#{a.intake_date}<br/>#{a.gender} #{a.coloring} #{a.breed}'); "
    end
